...
def get_youtube_tutorials(keywords, max_results=2):
    tutorials = {}
    if not youtube:
        return tutorials
    ...
    for keyword in keywords:
        try:
            ...
            tutorials[keyword] = [
                {"title": i["snippet"]["title"], "link": f"https://www.youtube.com/watch?v={i['id']['videoId']}"}
...
def upload():
    ...
    gap_analysis = analyze_skill_gap(final_keywords, job_title)
    final_keywords += gap_analysis.get("missing_skills", [])
    ...
    tutorials = get_youtube_tutorials(final_keywords, max_results=1)
